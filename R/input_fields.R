parse_input_txt = function(txt) {
  ...
  fields = lapply(seq_along(txt), function(i) {
    ...
    if (has.eq[i]) {
      ...
      list(var=var, val=val, type=ifelse("text","longtext"))
    } else if (!has.bracket[i]) {
      var = trimws(str)
      long = endsWith(var,"+")
      if (long) var = substring(var,1,nchar(var)-1) %>% trimws()
      list(var=var, val="", type=ifelse("text","longtext"))
    } else {
      var = str.left.of(str,"[") %>% trimws()
...
input_field_ui = function(field,i=1, id=paste0("input_",i), value=NULL) {
  ...
  if (field$type == "text") {
    ...
    selectInput(id,label = field$var,value = value,choices = field$choices)
  }
}

input_fields_ui = function(fields, values=NULL) {
  restore.point("input_fields_ui")
  uis = lapply(seq_along(inputs), function(i) {
    input_field_ui(inputs[[i]],i=i, value=values[[i]])
  })
}
